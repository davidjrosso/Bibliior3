...
from app.repositories import cuota_repository, socio_repository
...
def generar(conn, data: dict) -> dict:
    ...
    socios = socio_repository.list_generation_targets(conn, periodo)
    creadas = 0
    timestamp = now_iso()
    for socio in socios:
        monto = config_model.cuota_monto(conn, socio["estado"])
        try:
            cuota_repository.insert_pending(conn, socio["id"], periodo, monto, timestamp)
            creadas += 1
        except Exception:
            continue
...
def control_generacion(conn, periodo: str) -> dict:
    ...
    total_cuotas = cuota_repository.count_all(conn)
...
def _contar_socios_objetivo(conn, periodo: str) -> int:
    return socio_repository.count_generation_targets(conn, periodo)


def _contar_cuotas_objetivo(conn, periodo: str) -> int:
    return cuota_repository.count_generation_period(conn, periodo)
...
def listar_faltantes_generacion(conn, query: dict) -> dict:
    ...
    socios = socio_repository.list_generation_targets(conn, hasta)
    existentes = cuota_repository.list_existing_generation_pairs(conn, desde, hasta)
...
def crear(conn, data: dict) -> int:
    ...
    timestamp = now_iso()
    return cuota_repository.insert_custom_pending(
        conn,
        socio_id,
        periodo,
        monto,
        str(data.get("observacion", "")),
        timestamp,
    )
...
def actualizar(conn, cuota_id: int, data: dict) -> None:
    ...
    rowcount = cuota_repository.update_details(
        conn,
        cuota_id,
        periodo,
        monto,
...
def eliminar(conn, cuota_id: int) -> None:
    caja_model.eliminar_cobro_cuota(conn, cuota_id)
    if cuota_repository.delete(conn, cuota_id) == 0:
        raise LookupError("Cuota no encontrada")
...
def marcar_pagada(conn, cuota_id: int, fecha_pago: str | None = None, medio_pago: str = "efectivo") -> None:
    ...
    row = cuota_repository.find_by_id(conn, cuota_id)
    if not row:
        raise LookupError("Cuota no encontrada")
    if row["estado"] == "pagada":
        raise ValueError("La cuota ya esta pagada.")
    if cuota_repository.mark_paid(conn, cuota_id, fecha, now_iso()) == 0:
        raise LookupError("Cuota no encontrada")
...
def marcar_pagadas(conn, data: dict) -> dict:
    ...
    if medio_pago not in caja_model.MEDIOS_PAGO:
        raise ValueError("Medio de pago invalido.")
    rows = cuota_repository.list_statuses(conn, ids)
...
def pago_adelantado(conn, data: dict) -> dict:
    ...
    pagadas_existentes = cuota_repository.list_paid_periods(conn, socio_id, periodos)
    ...
    for periodo in periodos:
        monto = config_model.cuota_monto(conn, socio["estado"])
        row = cuota_repository.find_by_socio_period(conn, socio_id, periodo)
        if row:
            cuota_id = row["id"]
            cuota_repository.mark_existing_as_paid_advance(conn, cuota_id, monto, fecha_pago, timestamp)
        else:
            cuota_id = cuota_repository.insert_paid_advance(conn, socio_id, periodo, monto, fecha_pago, timestamp)
            creadas += 1
        caja_model.registrar_cobro_cuota(conn, cuota_id, medio_pago)
        ids.append(cuota_id)
        pagadas += 1
...
def marcar_pendiente(conn, cuota_id: int) -> None:
    if cuota_repository.mark_pending(conn, cuota_id, now_iso()) == 0:
        raise LookupError("Cuota no encontrada")
    caja_model.eliminar_cobro_cuota(conn, cuota_id)
...
def cuotas_para_imprimir(conn, periodo: str, cobrador: int):
    ...
    return cuota_repository.list_for_print(conn, periodo, cobrador, limite_moroso, order)
